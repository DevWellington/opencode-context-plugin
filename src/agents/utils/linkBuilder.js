...
export function generateKeywordLinks(options) {
  ...
  // Define target files with correct paths (vault-root-relative)
  // Vault root is the project directory, so wiki-links must include .opencode/context-session prefix
  const targets = [
...
export function addKeywordNavigation(context) {
  ...
  // All paths are vault-root-relative for Obsidian compatibility
  // Vault root is the project directory, so wiki-links must include .opencode/context-session prefix
  switch (type) {
    case 'daily':
