...
class ContextPlugin {
  constructor(input) {
    ...
    debugLog(`[context-plugin] ContextPlugin instantiated for: ${this.directory}`);
  }

  async event(eventInput) {
    debugLog(`[context-plugin] RAW EVENT received: ${JSON.stringify(eventInput)}`);
    const event = eventInput?.event || eventInput;
    const eventType = event?.type;

    debugLog(`[context-plugin] Parsed eventType: ${eventType}`);
    if (!eventType) return;
